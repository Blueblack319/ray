...
from tf2_data import get_preprocess_fn
...
color_jitter = ColorJitter(0.8*1.0, 0.8*1.0, 0.8*1.0, 0.2*1.0)
rnd_color_jitter = RandomApply([color_jitter], p=0.8)
...
base_transform = get_complete_transform(output_shape=output_shape, kernel_size=kernel_size)
...
def applyTransform_to_tensor(row):
    row["image"] = to_tensor(row["image"])
    return row
def applyTransform_rnd_crop(row):
    row["image"] = rnd_crop(row["image"])
    return row
def applyTransform_rnd_flip(row):
    row["image"] = rnd_flip(row["image"])
    return row
def applyTransform_rnd_color_jitter(row):
    row["image"] = rnd_color_jitter(row["image"])
    return row
def applyTransform_rnd_gray(row):
    row["image"] = rnd_gray(row["image"])
    return row
def applyTransform_rnd_gaussian_blur(row):
    row["image"] = rnd_gaussian_blur(row["image"])
    return row
...
if __name__ == "__main__":
    ...
    if args.data_root is not None:
        ...
        #tf_preprocess = get_preprocess_fn();
        #tf_dataset = tf.keras.preprocessing.image_dataset_from_directory(args.data_root).unbatch()
        ## tf_dataset = tf_dataset.map(lambda img, label: (tf_crop_and_flip(img), label))
        #tf_dataset = tf_dataset.map(lambda img, label: (tf_preprocess(img), label), num_parallel_calls=tf.data.experimental.AUTOTUNE)
